...
class ProductosController:
    ...
    def on_show(self):
        self.load_categories_combo(); self.load_products()
    ...
    def load_categories_combo(self):
        ...
        if values and not self.view.categoria.get(): self.view.categoria.set(values[0])
    ...
    def load_products(self, productos=None):
        ...
        for p in productos:
            self.view.tabla.insert("", "end", values=(p.id_producto, p.nombre, p.categoria, p.marca, money(p.precio), p.stock, p.stock_minimo, p.estado))
    ...
    def create_product(self):
        try:
            ...
            self.clear_form(); self.load_categories_combo(); self.load_products(); messagebox.showinfo("Productos", "Producto registrado.")
        except Exception as e: messagebox.showerror("Error", str(e))

    def update_product(self):
        try:
            self.app_controller.services.producto_service.actualizar_producto(self.view.id_producto.get(), **self._form())
            self.clear_form(); self.load_categories_combo(); self.load_products(); messagebox.showinfo("Productos", "Producto actualizado.")
        except Exception as e: messagebox.showerror("Error", str(e))
    ...
    def fill_form_from_selection(self):
        ...
        self.clear_form()
        self.view.id_producto.insert(0, producto.id_producto); self.view.nombre.insert(0, producto.nombre)
        self.view.categoria.set(f"{producto.id_categoria} | {producto.categoria}")
        self.view.marca.insert(0, producto.marca); self.view.precio.insert(0, str(producto.precio))
        self.view.stock.insert(0, str(producto.stock)); self.view.stock_minimo.insert(0, str(producto.stock_minimo))
        self.view.estado.set(producto.estado); self.view.descripcion.insert(0, producto.descripcion)

    def clear_form(self):
        for e in [self.view.id_producto, self.view.nombre, self.view.marca, self.view.precio, self.view.stock, self.view.stock_minimo, self.view.descripcion]: e.delete(0,"end")
